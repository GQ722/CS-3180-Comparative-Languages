...
# Scanner generation
tokens = ('STR', 'CLASSNAME', 'MAKECLASS', 'SUBCLASS', 'CONCAT', 'COMPARE', 'VARNAME', )
...
def t_COMPARE(t): r'''compare'''; return t
...
class ConcatNode(Node):
    ...
    def doit(self):
        result = selfNode.leftNode.doit() + selfNode.rightNode.doit()
        print(resutl)
        return result
...
def p_statement_subclass(p):
    " statement : SUBCLASS CLASSNAME statement_list '.' "
    p[0] = SubclassNode(p[2], p[3], p[4])

    def p_statement_assignclass(p):
        " statement : SUBCLASS CLASSNAME statement_list '.' "
        p[0] = AssignVarNode(p[2], p[3])
